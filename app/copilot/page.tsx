          ...
          <div className="mt-8 space-y-3 border-t border-line pt-6">
            {copilotSteps.map((step, index) => (
              ...
              >
                ...
                <p className="text-[1rem] leading-7 text-ink/76">{step}</p>
              </div>
            ))}
          </div>
        </Panel>
        ...
        <Panel className="space-y-5">
          ...
          <div className="grid gap-4 lg:grid-cols-[0.68fr_0.32fr]">
            <div className="rounded-[22px] border border-line bg-paper p-5">
              ...
              <p className="mt-3 text-[1.02rem] leading-8 text-ink/78">
              ...
              <div className="mt-6 grid gap-3 border-t border-line pt-5 sm:grid-cols-3">
                {[
                  ...
                  <div key={label}>
                    ...
                    <p className="mt-2 text-[1rem] font-medium text-ink">{value}</p>
                  </div>
                ))}
              </div>
            </div>
              ...
              <div className="mt-4 space-y-3">
                {[
                  ...
                  <div key={label} className="border-b border-line pb-3 last:border-b-0 last:pb-0">
                    <p className="text-[1rem] font-medium text-ink">{label}</p>
                    <p className="mt-1 text-[0.98rem] text-ink/72">{value}</p>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            {[
              ...
              >
                ...
                <p className="mt-3 text-[1rem] font-medium leading-6 text-ink">
                  {item.value}
                </p>
              </div>
            ))}
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            {[
              ...
              >
                ...
                <p className="mt-4 text-[1rem] font-medium text-ink">{item.title}</p>
                <p className="mt-2 text-[0.98rem] leading-7 text-ink/74">{item.body}</p>
              </div>
            ))}
          </div>
        </Panel>
